...
if (promptElement && characterCount) {
    promptElement.addEventListener("input", function () {
        characterCount.innerText = promptElement.value.length;
    });
}
...
async function sendPrompt() {
    ...
    // --------------------------------------------------------
    if (!prompt) {
        showStatus("blocked", "🚫 Please enter a prompt.");
        resetPipeline();
        return;
    }
    ...
    // --------------------------------------------------------
    resetPipeline();
    responseBox.className = "response hidden";
    ...
    showStatus("success", "🛡️ Checking input guardrail...");
    ...
    try {
        ...
        // ====================================================
        if (data.status === "blocked" && data.stage === "input_guardrail") {
            stepInput.classList.remove("active");
            stepInput.classList.add("blocked");

            showStatus("blocked", "🚫 Input blocked by guardrail.");
        ...
        showStatus("success", "🤖 Gemini is generating a response...");

        // ====================================================
        // OUTPUT GUARDRAIL
        // ====================================================
        stepGemini.classList.remove("active");
        stepGemini.classList.add("success");
        ...
        // OUTPUT BLOCKED
        // ====================================================
        if (data.status === "blocked" && data.stage === "output_guardrail") {
            stepOutput.classList.remove("active");
            stepOutput.classList.add("blocked");

            showStatus("blocked", "🚫 Output blocked by guardrail.");
            showSecurityResult(data.stage, data.category, data.message);
        ...
        // ====================================================
        stepOutput.classList.remove("active");
        stepOutput.classList.add("success");

        stepResponse.classList.add("success");
        showStatus("success", "✅ Response passed all guardrails.");
        ...
        console.error(error);
        showStatus("error", "❌ Unable to connect to the server.");
    } finally {
        sendButton.disabled = false;
        sendButton.innerText = "🚀 Send Prompt";
    }
...
function showSecurityResult(stage, category, message) {
    securityBox.className = "security-box";
    stageValue.innerText = formatText(stage);
...
function resetPipeline() {
    const steps = [stepInput, stepGemini, stepOutput, stepResponse];
    steps.forEach(function (step) {
        if (step) {
            step.classList.remove("active", "success", "blocked");
        }
    });
